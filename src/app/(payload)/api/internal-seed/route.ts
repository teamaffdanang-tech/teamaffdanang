 ...
 *
 * POST is scoped to the Viwoods e-ink tablet batch ONLY (3 new products, 1 new
 * brand, 1 new retailer, plus the existing `e-ink-tablets` category upserted
 * with its unchanged imported.ts value). No existing product is re-seeded.
 * Viwoods uses the existing USD currency — no migration involved.
 *
...
const NEW_PRODUCT_SLUGS = new Set(['viwoods-aipaper', 'viwoods-aipaper-mini', 'viwoods-aipaper-reader'])

const viwoodsBatch = (): SeedDataset => ({
  categories: importedDataset.categories.filter((c) => c.slug === 'e-ink-tablets'),
  occasions: [],
  brands: importedDataset.brands.filter((b) => b.slug === 'viwoods'),
  retailers: importedDataset.retailers.filter((r) => r.slug === 'viwoods'),
  authors: [],
