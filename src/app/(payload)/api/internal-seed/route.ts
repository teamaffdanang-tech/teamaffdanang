 ...
 *
 * POST is scoped to the AlgoLaser batch ONLY (7 new products, 1 new brand, 1
 * new retailer, plus the new `laser-engravers` category). No existing product
 * is re-seeded. AlgoLaser uses the existing USD currency — no migration.
 *
...
const NEW_PRODUCT_SLUGS = new Set([
  'algolaser-t150-pro', 'algolaser-diy-kit-mini', 'algolaser-diy-kit-10w-5w', 'algolaser-diy-kit-mk3',
  'algolaser-pixi-10w', 'algolaser-alpha-mk2-20w', 'algolaser-alpha-mk2-40w',
])

const algolaserBatch = (): SeedDataset => ({
  categories: importedDataset.categories.filter((c) => c.slug === 'laser-engravers'),
  occasions: [],
  brands: importedDataset.brands.filter((b) => b.slug === 'algolaser'),
  retailers: importedDataset.retailers.filter((r) => r.slug === 'algolaser'),
  authors: [],
