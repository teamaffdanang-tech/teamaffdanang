...
/** Temporary wave-2 seed endpoint. Seeds ONE merchant by slug prefix. Gated by
 *  MIGRATION_TEMP_SECRET. Delete route + secret after the wave. */
export const maxDuration = 300
export const dynamic = 'force-dynamic'

const ALLOWED = new Set(['ikko-', 'cys-', 'funwater-', 'moes-', 'carputech-'])
...
const buildSubset = (prefix: string): SeedDataset => {
  ...
  return {
    ...
    occasions: [], brands: importedDataset.brands.filter((b) => brands.has(b.slug)),
    retailers: importedDataset.retailers.filter((r) => rets.has(r.slug)),
    authors: [], products, coupons: [], blogPosts: [],
  }
}
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ status: 'ok', prefix, seeded: { categories: subset.categories.length, brands: subset.brands.length, retailers: subset.retailers.length, products: subset.products.length }, productsNowInDb: inScope.length })
}
...
export async function GET(req: NextRequest) {
  ...
  if (prefix) {
    ...
    const slugs = new Set<string>(); const affs = new Set<string>(); const brandCount: Record<string, number> = {}; let problems = 0; const bad: string[] = []
    ...
    return NextResponse.json({ prefix, count: slugs.size, uniqueAff: affs.size, brandCount, problems, bad })
  }
