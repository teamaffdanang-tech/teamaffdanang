...
/**
 * Temporary wave seed endpoint — gated by MIGRATION_TEMP_SECRET. Seeds ONE
 * merchant at a time, scoped by product slug prefix (?prefix=wautton-), plus
 * only the category/brand/retailer those products reference. Never seeds the
 * whole dataset. Delete this route + the secret after the wave is applied.
 */
export const maxDuration = 300
export const dynamic = 'force-dynamic'

const ALLOWED = new Set(['wautton-', 'binize-', 'woodemon-'])
...
const buildSubset = (prefix: string): SeedDataset => {
  ...
  for (const p of products) {
    ...
    if (p.brandSlug) brands.add(p.brandSlug)
  ...
  return {
    ...
    retailers: importedDataset.retailers.filter((r) => rets.has(r.slug)),
    authors: [],
    products,
    coupons: [],
    blogPosts: [],
...
export async function POST(req: NextRequest) {
  ...
  const inScope = now.docs.filter((d) => String((d as { slug?: string }).slug || '').startsWith(prefix))
  return NextResponse.json({
    status: 'ok', prefix,
    seeded: { categories: subset.categories.length, brands: subset.brands.length, retailers: subset.retailers.length, products: subset.products.length },
    productsNowInDb: inScope.length,
  })
}
...
export async function GET(req: NextRequest) {
  ...
  if (prefix) {
    ...
    const brandCount: Record<string, number> = {}; const catCount: Record<string, number> = {}
    let problems = 0; const bad: string[] = []
    for (const d0 of docs) {
      ...
      const gallery = (d.gallery as unknown[]) || []
      const aff = String(rl.affiliateUrl || ''); const off = String(d.officialUrl || '')
      const dupSlug = slugs.has(slug); slugs.add(slug)
      const dupAff = affs.has(aff); affs.add(aff)
      if (brand) brandCount[brand] = (brandCount[brand] || 0) + 1
      for (const c of cats) catCount[c] = (catCount[c] || 0) + 1
      const ok = d._status === 'published' && cats.length > 0 && ret.slug && rl.currency === 'USD' &&
        !aff.includes('?') && aff === off && (ratings.overall === undefined || ratings.overall === null) &&
        ((d.occasions as unknown[]) || []).length === 0 && gallery.length >= 1 && !dupSlug && !dupAff
      if (!ok) { problems++; bad.push(slug) }
    }
    return NextResponse.json({ prefix, count: slugs.size, uniqueSlugs: slugs.size, uniqueAff: affs.size, brandCount, catCount, problems, bad })
  }
  return NextResponse.json({ error: 'use ?counts=1, ?verify=<prefix>, ?images=<prefix>' }, { status: 400 })
}
