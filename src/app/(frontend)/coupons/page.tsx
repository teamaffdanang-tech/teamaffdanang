...
import { getActiveCoupons, resolveCouponDisplayProduct } from "@/lib/coupons";
...
export default async function CouponsPage() {
  ...
  // Brand-scope coupons often have no linkedProduct of their own — resolve a
  // display-anchor product (image/title for the card) so CouponCard/JSON-LD
  // never silently drop a coupon just because it applies sitewide.
  const displayCoupons = await Promise.all(
    coupons.map(async (coupon) => {
      const displayProduct = await resolveCouponDisplayProduct(payload, coupon);
      return { ...coupon, linkedProduct: displayProduct ?? coupon.linkedProduct };
    }),
  );
